...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def func(str):
        word_list = split(remove_punctuation(str))
        for i in word_list:
            if lower(i) in topic:
                return True
        return False
    ...
    if typed_words == []:
        ...
        for index,word in enumerate(typed_words):
            if index < len(source_words):
                if word == source_words[index]:
                    score += 1
        return score*100/len(typed_words)
...
def sphinx_fixes(typed, source, limit):
    ...
    for index,alph in enumerate(typed):
        if alph != source[index]:
            result += 1
            if result > limit:
                return result
    if len(typed) < len(source):
        result += len(source) - len(typed)
    return result
    # END PROBLEM 6
...
def fastest_words(game):
    ...
    for i in word_indices:
        l1 = [j[i] for j in get_all_times(game)]
        result[l1.index(min(l1))].append(get_word(game,i))
    return result
    # END PROBLEM 10
