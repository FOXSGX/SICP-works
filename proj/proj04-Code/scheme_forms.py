...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        env.define(signature.first, do_lambda_form(expr,env))
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        validate_form(element,2,2)
        names = Pair(element.first,names)
        values = Pair(scheme_eval(element.rest.first,env),values)
