...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    a = make_city(0,lat,lon)
    if distance(a,city1) < distance(a,city2):
...
def sprout_leaves(t, values):
    ...
    for i in branches(t):
        if is_leaf(i):
            new_branch += [tree(label(i),[tree(i) for i in values])]
        else:
            new_branch += [sprout_leaves(i,values)]
...
def insert_items(lst, entry, elem):
    ...
    """
    index_list = []
    for index,thing in enumerate(lst):
        if thing == entry:
            index_list.append(index)
    for index,thing in enumerate(index_list):
        lst.insert(thing+index+1,elem)
    return lst
