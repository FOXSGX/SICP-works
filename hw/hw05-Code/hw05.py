...
def make_withdraw(balance, password):
    ...
    1
    def withdraw(cin,pw):
        nonlocal balance,wrong_times
        if wrong_times == 3:
            return "Your account is locked. Attempts: {0}".format(l)
        elif pw != password:
            wrong_times += 1
            l.append(pw)
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    if type(ans) == str:
        ...
        def joint(cin,pw):
            if pw == new_pass:
                return withdraw(cin,old_pass)
            return withdraw(cin,pw)
        return joint
...
def distribute_parfait(n, k):
    ...
    if n < k:
        ...
        for i in range(1,n-k+2):
            for j in distribute_parfait(n-i,k-1):
                yield [i] + j
